// calc remains for each month, return arr of sums for each month

const getMonthsRemains = (allDocs, howManyMonths, currentBalance) => {
  const start = new Date(); // get now
  start.setDate(1); // set 'now' to month start
  start.setHours(0, 0, 0, 0); // reset hours too
  ...
  allDocs.forEach((doc) => {
    ...
    const diff = yearDiff * 12 + monthDiff; // convert yearDiff to months and add monthDiff = get total monthDiff
